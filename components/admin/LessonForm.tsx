...
export default function LessonForm({ lessonId, onSaved }: { lessonId?: number; onSaved?: () => void }) {
  ...
  const inputCls = "w-full border border-[#22304C] rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:border-[#2C3B5C] transition-colors"
  const labelCls = "text-xs font-medium text-[#8FA9C6] block mb-1.5"

  if (loadingLesson) {
    return <div className="text-sm text-[#8FA9C6] p-8 text-center">Đang tải bài học...</div>
  }

  return (
    <div className="space-y-5">
      {/* Card 1: Thông tin cơ bản */}
      <div className="bg-[#0E1526] rounded-2xl border border-[#22304C] p-5">
        ...
        <label className={labelCls}>YouTube video ID 2 <span className="text-[#8FA9C6] font-normal">(video phụ, để trống nếu không có)</span></label>
        <input className={`${inputCls} mb-4`}
          placeholder="dQw4w9WgXcQ"
      ...
      <div className="bg-[#0E1526] rounded-2xl border border-[#22304C] p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-[#8FA9C6] mt-0.5">{mcqs.length} câu</p>
          </div>
          <button
        ...
        <div className="space-y-3">
          {mcqs.map((mcq, qi) => (
            <div key={qi} className="bg-[#141E36] rounded-xl p-4">
    ...
    return (
      <div key={oi}
        ...
        className={`flex items-center gap-2 border rounded-lg px-2.5 py-1.5 cursor-pointer transition-all ${
          isCorrect ? 'border-[rgba(74,222,128,0.4)] bg-[rgba(74,222,128,0.12)]' : 'border-[#22304C] bg-[#0E1526] hover:border-[#2C3B5C]'
        }`}
      >
        <span className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
          isCorrect ? 'border-[rgba(74,222,128,0.5)] bg-[#4ADE80]' : 'border-[#2C3B5C]'
        }`}>
          {isCorrect && <i className="ti ti-check text-white" style={{fontSize:'11px'}} />}
        </span>
        <span className={`text-xs font-medium flex-shrink-0 ${isCorrect ? 'text-[#4ADE80]' : 'text-[#8FA9C6]'}`}>
          {['A','B','C','D','E','F'][oi]}
        </span>
        <input
        ...
        {mcq.options.length > 2 && (
          <button onClick={e => {
            ...
            setMcqs(u)
          }} className="text-[#5F7796] hover:text-[#F87171] flex-shrink-0">
...
{mcq.options.length < 6 && (
  <button
    ...
    className="mt-2 text-xs text-[#8FA9C6] hover:text-[#C6D5E8] flex items-center gap-1"
  >
      ...
      <div className="bg-[#0E1526] rounded-2xl border border-[#22304C] p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-[#8FA9C6] mt-0.5">{essays.length} câu</p>
          </div>
          <button
        ...
        <div className="space-y-2">
          {essays.map((eq, qi) => (
            <div key={qi} className="flex items-start gap-2">
              <span className="w-6 h-6 rounded-md bg-[#1A2542] flex items-center justify-center text-xs text-[#8FA9C6] flex-shrink-0 mt-1.5">
                {qi + 1}
              </span>
              <textarea rows={2} className="flex-1 min-w-0 border border-[#22304C] rounded-lg px-3 py-2 text-sm resize-y"
              ...
              {essays.length > 0 && (
                <button onClick={() => setEssays(essays.filter((_, i) => i !== qi))} className="text-[#5F7796] hover:text-[#F87171] flex-shrink-0 mt-2">
                  <i className="ti ti-x" style={{fontSize:'14px'}} />
                </button>
              )}
            </div>
      ...
      <div className="bg-[#0E1526] rounded-2xl border border-[#22304C] p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-[#8FA9C6] mt-0.5">{tfGroups.length} nhóm</p>
          </div>
          <button
        ...
        <div className="space-y-4">
          {tfGroups.map((group, gi) => (
            <div key={gi} className="bg-[#141E36] rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-medium text-[#8FA9C6]">Nhóm {gi + 1}</span>
                <button onClick={() => setTfGroups(tfGroups.filter((_, i) => i !== gi))} className="text-[#5F7796] hover:text-[#F87171]">
                  <i className="ti ti-x" style={{fontSize:'14px'}} />
                </button>
              </div>

              <input className="w-full border border-[#22304C] rounded-lg px-3 py-2 text-sm mb-3 bg-[#0E1526]"
              ...
              <div className="space-y-2">
                {group.items.map((item, ii) => (
                  <div key={ii} className="flex items-center gap-2">
                    <span className="w-6 h-6 rounded-md bg-[#0E1526] border border-[#22304C] flex items-center justify-center text-xs text-[#8FA9C6] flex-shrink-0">
                      {ii + 1}
                    </span>
                    <input className="flex-1 min-w-0 border border-[#22304C] rounded-lg px-3 py-2 text-sm bg-[#0E1526]"
                      ...
                      {[true, false].map(val => (
                        <button key={String(val)}
                          ...
                          className={`px-3 py-2 rounded-lg text-xs font-semibold transition-colors ${
                            item.correct === val
                              ? (val ? 'bg-[#4ADE80] text-white' : 'bg-[#F87171] text-white')
                              : 'bg-[#0E1526] border border-[#22304C] text-[#8FA9C6]'
                          }`}>
                          {val ? 'Đúng' : 'Sai'}
                        </button>
                      ))}
...
{/* Card 4b: Tài liệu đính kèm */}
      <div className="bg-[#0E1526] rounded-2xl border border-[#22304C] p-5">
        ...
        <p className="text-xs text-[#8FA9C6] mb-3">Học viên sẽ xem được tài liệu ngay trong bài học qua Google Docs Viewer.</p>

        {attachmentUrl && (
          <div className="flex items-center gap-2 mb-3 p-3 bg-[rgba(96,165,250,0.12)] rounded-xl">
            <i className="ti ti-file-type-pdf text-[#60A5FA]" style={{fontSize:'16px'}} />
            ...
            <button onClick={() => { setAttachmentUrl(''); setAttachmentFile(null) }}
              className="text-[#8FA9C6] hover:text-[#F87171] flex-shrink-0">
              <i className="ti ti-x" style={{fontSize:'14px'}} />
            </button>
          </div>
        )}

        <label className="block border border-dashed border-[#22304C] rounded-xl px-4 py-3 cursor-pointer hover:border-[#2C3B5C] transition-colors">
          <div className="flex items-center gap-2 text-sm text-[#8FA9C6]">
        ...
        {uploadingPdf && <p className="text-xs text-[#8FA9C6] mt-2">Đang upload PDF...</p>}
      </div>
{/* Card 4b: Recap bài học */}
      <div className="bg-[#0E1526] rounded-2xl border border-[#22304C] p-5">
        <div className="mb-3">
          <p className="text-sm font-medium">Nội dung Recap</p>
          <p className="text-xs text-[#8FA9C6] mt-0.5">Hiển thị sau khi học viên hoàn thành bài — tóm tắt điểm quan trọng cần nhớ.</p>
        </div>
      ...
      <div className="bg-[#0E1526] rounded-2xl border border-[#22304C] p-5">
        <label className="flex items-center gap-3 cursor-pointer">
          <input
            ...
            className="w-4 h-4 rounded border-[#2C3B5C]"
          />
          <div>
      ...
      {success && (
        <div className="bg-[rgba(74,222,128,0.12)] border border-[rgba(74,222,128,0.22)] rounded-xl px-4 py-3 flex items-center gap-2">
          <i className="ti ti-check text-[#4ADE80]" />
