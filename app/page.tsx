...
setSubmissionStatusMap(latestStatusMap)
  ...
  // Badge tự động cho module không có tên riêng (dạng "module-{id}") — không cần sửa code/upload ảnh mỗi lần thêm module mới
  const dynamicBadgeDefs: BadgeDef[] = badges
    .filter(bt => /^module-\d+$/.test(bt) && !badgeDefs.some(b => b.type === bt))
    .map(bt => {
      const moduleId = parseInt(bt.replace('module-', ''), 10)
      const mod = modules.find(m => m.id === moduleId)
      const name = mod?.name ?? 'module này'
      return {
        type: bt,
        label: `Hoàn thành: ${name}`,
        emoji: '🏆',
        bg: '#FBF7EE',
        color: '#B8860B',
        desc: `Bạn đã hoàn thành toàn bộ "${name}" — xuất sắc!`,
      }
    })
  const allBadgeDefs: BadgeDef[] = [...badgeDefs, ...dynamicBadgeDefs]
  ...
  useEffect(() => {
    ...
    const newBadge = allBadgeDefs.slice().reverse().find(b => badges.includes(b.type) && !seen.includes(b.type))
    if (newBadge) {
      setBadgePopup(newBadge)
      localStorage.setItem(seenKey, JSON.stringify([...seen, newBadge.type]))
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, badges, modules])
  ...
  {b.imageUrl ? (
    ...
    <span className="text-lg" style={{ opacity: earned ? 1 : 0.3, filter: earned ? 'none' : 'grayscale(100%)' }}>
      {b.emoji}
    </span>
  )}
</div>
...
{isDone && (
  ...
  {badgePopup.imageUrl ? (
    ...
    <span className="text-5xl">{badgePopup.emoji}</span>
  )}
</div>
